...
class details extends State<DetailsBody> {
  ...
  Future<void> _tryShoeSize() async {
    // Open the camera
    final pickedFile = await _picker.pickImage(source: ImageSource.camera);

    if (pickedFile != null) {
      // Simulate foot detection and randomly assign a size
      List<String> sizes = ['6.0', '7', '8.0', '9']; // List of shoe sizes
      Random random = Random();
      int randomIndex = random.nextInt(sizes.length);
      String detectedSize = sizes[randomIndex];

      // Display the detected size to the user
      showDialog(
        context: context,
        builder: (context) {
          return AlertDialog(
            ...
            title: Text(
              'Detected Size',
              style: TextStyle(color: Colors.white), // Optional: Change text color
            ),
            content: Text(
              'Your detected shoe size is: $detectedSize',
              style: TextStyle(color: Colors.white), // Optional: Change text color
            ),
            actions: [
              TextButton(
                onPressed: () {
                  // Add the item to cart with the detected size
                  AppMethods.addToCart(widget.model, context, size: detectedSize);
  ...
  materialButton(width, height) {
    return FadeAnimation(
      ...
      child: MaterialButton(
        ...
        onPressed: () {
          AppMethods.addToCart(widget.model, context, size: '7');
        },
        child: Text(
          "ADD TO BAG",
